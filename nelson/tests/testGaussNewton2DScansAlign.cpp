...
TEST_CASE("GaussNewton-MT-speedup", "GaussNewton-MT-speedup") {
  using ProblemType = SE2PoseSectionFD_BlockDense;
  constexpr int  solverType = nelson::solverCholeskyDense;
  ...
  for (int repeat = 0; repeat < 2; repeat++) {
    SECTION("repeat test for time " + std::to_string(repeat)) {
      ...
      Eigen::Vector3d noiseSigma = Eigen::Vector3d(0.01, 0.01, M_PI / 1000.0);
      bool fullEdges = true;
