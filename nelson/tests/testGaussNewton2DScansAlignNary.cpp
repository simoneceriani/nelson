...
class PointLineEdge : public nelson::EdgeNarySectionBaseCRPT<Section, 2, typename Section::EdgeUnaryAdapter, PointLineEdge<Section>> {
...
public:
  ...
  void update(bool hessians) override {
    const auto& modelPose = this->parameter(0);
    const auto& targetPose = this->parameter(1);
