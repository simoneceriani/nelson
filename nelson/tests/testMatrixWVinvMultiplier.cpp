...
template<int matWType, int matVType, int BR, int BC, int NBR, int NBC>
void templateTestFunc() {
  typename mat::MatrixBlockIterableTypeTraits<matVType, double, mat::ColMajor, BC, BC, NBC, NBC>::MatrixType Vinv;
  ...
  typename mat::MatrixBlockIterableTypeTraits<matWType, double, mat::RowMajor, BR, BC, NBR, NBC>::MatrixType W;

  mat::SparsityPattern<mat::RowMajor>::SPtr sp(new mat::SparsityPattern<mat::RowMajor>(NBRv, NBCv));
  if (matWType != mat::BlockDiagonal) {
    // 0 1 2 3 4
    // X - - X X
  ...
  nelson::MatrixWVinvMultiplier<matWType, matVType, double, BR, BC, NBR, NBC> mwvinv;
  ...
  nelson::DenseWrapper<matWType, double, mat::RowMajor, BR, BC, NBR, NBC> W_Wrap;

  W_Wrap.set(&W);
  DEBUGOUT std::cout << "W_Wrap" << std::endl << W_Wrap.mat() << std::endl << std::endl;
  nelson::DenseWrapper<matVType, double, mat::ColMajor, BC, BC, NBC, NBC> Vinv_Wrap;
  Vinv_Wrap.set(&Vinv);
  ...
  nelson::DenseWrapper<matWType, double, mat::RowMajor, BR, BC, NBR, NBC> WVinv_Wrap;
...
TEMPLATE_TEST_CASE_SIG("MatrixWVinvMultiplier-VDiagBlock", "[MatrixWVinvMultiplier-VDiagBlock]",
  ((int matWType, int BR, int BC, int NBR, int NBC), matWType, BR, BC, NBR, NBC),
...
{
  templateTestFunc< matWType, mat::BlockDiagonal, BR, BC, NBR, NBC>();
}


TEMPLATE_TEST_CASE_SIG("MatrixWVinvMultiplier-VDiagCoeff", "[MatrixWVinvMultiplier-VDiagCoeff]",
  ((int matWType, int BR, int BC, int NBR, int NBC), matWType, BR, BC, NBR, NBC),
  (mat::BlockDense, BRv, BCv, NBRv, NBCv),
  (mat::BlockSparse, BRv, BCv, NBRv, NBCv),
  (mat::BlockCoeffSparse, BRv, BCv, NBRv, NBCv),
...
{
  templateTestFunc< matWType, mat::SparseCoeffBlockDiagonal, BR, BC, NBR, NBC>();
}